...
def validate_username(username):
    ...
    if len(username.strip()) < 5:
        raise serializers.ValidationError("The username should be 5 or more characters long")
    return username

def validate_password(password):

    if len(password) < 7:
        raise serializers.ValidationError("The password should be 7 or more characters long")
...
def validate_contact(contact):
    ...
    if check_contact.exists():
        raise serializers.ValidationError(f"A user with that contact {contact} already exists")
